...
const int calibrationFactor1 = 744;  
const int calibrationFactor2 = 744;  
...
void setup() {
  ...
  scale_1.set_scale(calibrationFactor1);
  scale_2.set_scale(calibrationFactor2);
  scale_1.tare();
  scale_2.tare();
...
void loop() { 
  ...
  channel_change(channel_B);                //Get Channel B readings
  float channelB1 = measure_scale1() * 2;   //Output scaled since Channel B has less gain.
  float channelB2 = measure_scale2() * 2;   

  float totalWeight = channelA1 + channelA2 + channelB1 + channelB2;
  ...
  if ((totalWeight * KG_TO_LB) > 190){			
    overloadWarning();		  		                //When weight (in pounds) exceeds 190, warning is displayed. 
  } else {
    ...
    lcd.print("Total: ");
    lcd.setCursor(0, 1);
    lcd.print(displayWeight, decPlaces);		    //Prints display weight to 2dp.
    lcd.print(" " + unitStr + " ");
  } 
}
...
float measure_scale2(){
  float returnValue = initialWeight1 + (scale_2.get_units(1) / 1000.0);
