...
/*
    ...
    Description: 
      ...
      change the measuremnt units. Need to implement low battery indicator and overload indicator.
      
      1.7 (21/03/2025): Added debounce on button interrupts to prevent false triggering. This fixed the issue where units button would sometimes trigger both Unit and Tare ISR.
...
void loop() {
  ...
  if (totalWeight * KG_TO_LB > 190){			
    ...
    lcd.print("Total: ");
    lcd.setCursor(0, 1);
    lcd.print(displayWeight, decPlaces);		//prints display weight to 3dp.
    lcd.print(" " + unitStr + " ");
  }
