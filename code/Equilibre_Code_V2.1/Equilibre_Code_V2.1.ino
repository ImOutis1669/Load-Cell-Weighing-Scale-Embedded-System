...
/*
    ...
    Author: John Pritchard & Jeremy Antwi
    Date: 24/03/2025
    Version: 2.1
    ...
    Description: 
      ...
      2.1 (27/03/2025): Rewrote the main logic. Takes the highest reading and displays it now in an attempt to prevent fluctuations.
...
float division = 1;
float highest = 0;      //Stores highest value recorded
...
void loop() { 
  ...
  for (int x = 0; x < 10; x++){               //Repeats to get the highest value
    ...
    if (totalWeight > highest){
    highest = totalWeight;
    }
  }

  delay(3000);
  float displayWeight = highest -15.2;                //Takes weight of plate off
  if (unitMode == 0) {
      unitStr = "kg";         
      division = 1;
  } else if (unitMode == 1) {
      displayWeight = (displayWeight * KG_TO_GRAMS);
      unitStr = "g";
      division = KG_TO_GRAMS;
  } else if (unitMode == 2) {
      displayWeight = (displayWeight * KG_TO_LB);
      unitStr = "lb";
      division = KG_TO_LB;
  } else if (unitMode == 3) {
      displayWeight = (displayWeight * KG_TO_STONE);
      unitStr = "st";
      division = KG_TO_STONE;
  }

  if ((displayWeight/division) > 190){			    //Converts back to kg for comparison (as a value in grams will appear much larger to the computer)
    overloadWarning();		  		                //When weight exceeds 190, warning is displayed
  } else {
